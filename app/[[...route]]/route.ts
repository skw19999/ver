...
// ==========================================
// BasePath ကို အလွတ်ထားပါ (Root မှာ Run မယ်)
// ==========================================
const app = new Hono().basePath('')

const ACCESS_PASSWORD = "1234"; 

// 1. Root URL (/) ဝင်ရင် Login စစ်မယ်
app.get('/', (c) => {
...
// 2. Login Submit လုပ်ရင်
app.post('/login', async (c) => {
  const body = await c.req.parseBody();
  if (body.password === ACCESS_PASSWORD) {
    // Cookie မှတ်ပြီး Root ကို ပြန်ပို့မယ်
...
app.post('/create', async (c) => {
    ...
    // Link ထုတ်ပေးရင် domain.com/file.mp4 အတိုင်းထွက်မယ်
    const fullUrl = `${protocol}://${host}/${fileName}`;
    
    return c.json({ success: true, link: fullUrl });
});

// 4. Stream & Download Handler
...
// UI HTML Forms (Updated Actions to Root paths)
...
function renderApp() { return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Root Proxy</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-black text-white min-h-screen flex items-center justify-center p-4"><div class="w-full max-w-lg bg-gray-900 p-8 rounded-2xl border border-gray-800"><h1 class="text-2xl font-bold text-center mb-6">⚡ Proxy Gen</h1><div class="space-y-4"><input id="mf-link" type="text" placeholder="MediaFire / Direct URL" class="w-full bg-black border border-gray-700 rounded-lg px-4 py-2 text-white"/><input id="file-name" type="text" placeholder="Filename" class="w-full bg-black border border-gray-700 rounded-lg px-4 py-2 text-white"/><button onclick="saveLink()" id="btn" class="w-full bg-white text-black font-bold py-2 rounded-lg hover:bg-gray-200">Generate</button></div><div id="result" class="mt-6 hidden"><input id="final-link" readonly class="w-full bg-black border border-green-900 text-green-500 rounded px-2 py-1 text-sm mb-2"/><a id="test-btn" href="#" class="text-xs text-gray-400 underline">Test Download</a></div></div><script>async function saveLink(){const url=document.getElementById('mf-link').value;const name=document.getElementById('file-name').value;const btn=document.getElementById('btn');if(!url||!name)return alert("Fill all fields");btn.innerText="...";btn.disabled=true;try{const res=await fetch("/create",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({url,name})});const data=await res.json();if(data.success){document.getElementById('final-link').value=data.link;document.getElementById('test-btn').href=data.link;document.getElementById('result').classList.remove('hidden')}else alert(data.error)}catch(e){alert("Error")}btn.innerText="Generate";btn.disabled=false}</script></body></html>`;}
