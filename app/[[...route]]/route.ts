...
app.post('/create', async (c) => {
    try {
        const auth = getCookie(c, "auth_token");
        if (auth !== ACCESS_PASSWORD) return c.json({ success: false, error: "Unauthorized: Please Login again" }, 401);

        const body = await c.req.parseBody();
        const originalUrl = body.url as string;
        let fileName = body.name as string;

        if (!originalUrl || !fileName) return c.json({ success: false, error: "URL and Filename are required!" });

        // Database Connection Test
        try {
            // KV အလုပ်လုပ်မလုပ် စမ်းသပ်ခြင်း
            await kv.set('test_connection', 'ok');
        } catch (dbError: any) {
            console.error("KV Error:", dbError);
            return c.json({ success: false, error: "Database Error: " + dbError.message });
        }
        ...
        if (exists) return c.json({ success: false, error: "Filename already exists! Change name." });
        ...
        return c.json({ success: false, error: "Server Error: " + e.message });
    }
});
...
async function handleRequest(c: any, filename: string, method: string) {
    ...
    try {
        ...
        if (!fileData) return c.text("File Not Found in Database", 404);
            ...
            if (cachedLink) {
                ...
                try {
                    ...
                    if (match && match[1]) {
                        ...
                        return c.text("MediaFire Link Expired/Blocked", 404);
                    }
                } catch (e) {
...
function renderApp() { return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Proxy Gen</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-black text-white min-h-screen flex items-center justify-center p-4"><div class="w-full max-w-lg bg-gray-900 p-8 rounded-2xl border border-gray-800"><h1 class="text-2xl font-bold text-center mb-6">⚡ Proxy Gen</h1><div class="space-y-4"><input id="mf-link" type="text" placeholder="MediaFire / Direct URL" class="w-full bg-black border border-gray-700 rounded-lg px-4 py-2 text-white"/><input id="file-name" type="text" placeholder="Filename (e.g. MyMovie)" class="w-full bg-black border border-gray-700 rounded-lg px-4 py-2 text-white"/><button onclick="saveLink()" id="btn" class="w-full bg-white text-black font-bold py-2 rounded-lg hover:bg-gray-200">Generate</button></div><div id="result" class="mt-6 hidden"><input id="final-link" readonly class="w-full bg-black border border-green-900 text-green-500 rounded px-2 py-1 text-sm mb-2"/><a id="test-btn" href="#" class="text-xs text-gray-400 underline">Test Download</a></div></div><script>async function saveLink(){const url=document.getElementById('mf-link').value;const name=document.getElementById('file-name').value;const btn=document.getElementById('btn');if(!url||!name)return alert("Fill all fields");btn.innerText="Checking...";btn.disabled=true;try{const res=await fetch("/create",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams({url,name})});const data=await res.json();if(data.success){document.getElementById('final-link').value=data.link;document.getElementById('test-btn').href=data.link;document.getElementById('result').classList.remove('hidden')}else{alert("ERROR: "+data.error)}}catch(e){alert("NETWORK ERROR: "+e.message)}btn.innerText="Generate";btn.disabled=false}</script></body></html>`;}
